...
class BannerPagerSnapHelper extends BannerSnapHelper {
    private static final int MAX_SCROLL_ON_FLING_DURATION = 100; // ms

    // Orientation helpers are lazily created per LayoutManager.
    @Nullable
    private BannerOrientationHelper mVerticalHelper;
    @Nullable
    private BannerOrientationHelper mHorizontalHelper;
    ...
    public int findTargetSnapPosition(RecyclerView.LayoutManager layoutManager, int velocityX,
        ...
        final BannerOrientationHelper orientationHelper = getOrientationHelper(layoutManager);
    ...
    private int distanceToCenter(@NonNull View targetView, BannerOrientationHelper helper) {
        final int childCenter = helper.getDecoratedStart(targetView);//+ (helper.getDecoratedMeasurement(targetView) / 2);
        final int containerCenter = helper.getStartAfterPadding() + getTotalSpace() / 2;
     ...
     *                      {@link RecyclerView}.
     * @param helper        The relevant {@link BannerOrientationHelper} for the attached {@link RecyclerView}.
     * @return the child view that is currently closest to the center of this parent.
     */
    @Nullable
    private View findCenterView(RecyclerView.LayoutManager layoutManager,
                                BannerOrientationHelper helper) {
        ...
        final int center = helper.getStartAfterPadding() + getTotalSpace() / 2;
    ...
    @Nullable
    private BannerOrientationHelper getOrientationHelper(RecyclerView.LayoutManager layoutManager) {
        if (layoutManager.canScrollVertically()) {
    ...
    @NonNull
    private BannerOrientationHelper getVerticalHelper(@NonNull RecyclerView.LayoutManager layoutManager) {
        if (mVerticalHelper == null || mVerticalHelper.mLayoutManager != layoutManager) {
            mVerticalHelper = BannerOrientationHelper.createVerticalHelper(layoutManager);
        }
        return mVerticalHelper;
    }

    @NonNull
    private BannerOrientationHelper getHorizontalHelper(
            @NonNull RecyclerView.LayoutManager layoutManager) {
        if (mHorizontalHelper == null || mHorizontalHelper.mLayoutManager != layoutManager) {
            mHorizontalHelper = BannerOrientationHelper.createHorizontalHelper(layoutManager);
        }
